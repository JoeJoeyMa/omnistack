...
export function ShopRail() {
  ...
  const { cartCount, savedCount } = useShopState();
  ...
  return (
    <aside className="fixed inset-x-0 bottom-0 z-50 border-t border-black/8 bg-white/96 px-3 py-2 backdrop-blur lg:inset-x-auto lg:bottom-auto lg:left-0 lg:top-0 lg:h-screen lg:w-[76px] lg:border-r lg:border-t-0 lg:px-0 lg:py-5">
      <div className="flex items-center justify-between gap-2 lg:h-full lg:flex-col">
        <Link
          className="hidden h-11 w-11 items-center justify-center rounded-[16px] bg-[#111] font-semibold text-white lg:inline-flex"
          to="/"
        >
          S
        </Link>
        <nav className="grid flex-1 grid-cols-5 gap-1 lg:flex lg:flex-col lg:items-center lg:justify-center lg:gap-4">
          {navItems.map((item) => {
            ...
            const badge = item.to === "/cart" && cartCount > 0 ? String(cartCount) : undefined;
            return (
              <Link
                className={cn(
                  ...
                  active && "bg-[var(--shop-brand)] text-white shadow-[0_8px_22px_rgba(84,51,235,0.26)]",
                )}
                key={item.label}
                to={item.to}
              >
          ...
          <button
            className={cn(
              "relative inline-flex h-11 w-11 items-center justify-center rounded-full text-black/35 transition hover:scale-105 hover:text-black lg:h-12 lg:w-12",
            )}
            type="button"
          >
